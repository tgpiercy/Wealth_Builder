...
# ==============================================================================
#  TITAN STRATEGY APP (v61.4 Master Integration)
# ==============================================================================
...
st.title(f"🛡️ Titan Strategy v61.4 ({current_user.upper()})")
st.caption("Institutional Protocol: Full Rotation Sync & Split RSI Colors")
...
# --- ZIG ZAG ENGINE (EXPANDED TO FIX SYNTAX ERROR) ---
...
if st.session_state.run_analysis:
    ...
    if mode == "Scanner":
        ...
        if spy is not None:
            if vix is not None:
                v = vix.iloc[-1]['Close']
                # Correcting to 11-point logic based on screenshot 7/11
                s = "NORMAL" if v < 17 else ("CAUTIOUS" if v < 20 else "PANIC")
                v_pts = 5 if v < 17 else (4 if v < 20 else (3 if v < 25 else (1 if v < 30 else 0)))
                mkt_score += v_pts
                h_rows.append({"Indicator": f"VIX Level ({v:.2f})", "Status": s})
            
            s_c = spy['Close'].iloc[-1]; s18 = calc_sma(spy['Close'], 18); s8 = calc_sma(spy['Close'], 8)
            s_checks = {"SPY > 18": s_c > s18.iloc[-1], "SPY 18 Rising": s18.iloc[-1] >= s18.iloc[-2], "SPY 8 Rising": s8.iloc[-1] > s8.iloc[-2]}
            ...
            if rsp is not None:
                ...
                r_checks = {"RSP > 18": r_c > r18.iloc[-1], "RSP 18 Rising": r18.iloc[-1] >= r18.iloc[-2], "RSP 8 Rising": r8.iloc[-1] > r8.iloc[-2]}
        ...
        for t in scan_list:
            ...
            if bench_tick in master_data:
                rs_s = df['Close'] / master_data[bench_tick]['Close']; rs_18 = calc_sma(rs_s, 18)
            ...
            # WEAKENING LOCK
            if "WEAKENING" in rrg_status and "BUY" in action: action = "CAUTION"
            
            # --- SPLIT RSI COLOR LOGIC ---
            r5_full = calc_rsi(df['Close'], 5); r20_full = calc_rsi(df['Close'], 20)
            r5 = r5_full.iloc[-1]; r20 = r20_full.iloc[-1]; is_rising = r5 > r5_full.iloc[-2]
    ...
    if mode == "Sector Rotation":
        ...
        if nav == "Indices":
            ...
            # INJECT SPY WHEN COMPARING TO IEF
            ...
            if st.button("Run"):
                ...
                st.plotly_chart(plot_rrg_chart(r, m, tc.RRG_INDICES, f"Indices vs {bench}", is_dark), use_container_width=True)
